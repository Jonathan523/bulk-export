...
func export(text, output string, selectedKeys []string) error {
	chars := []rune(text)
	if len(chars) == 0 {
		return fmt.Errorf("no characters provided")
	}
...
func main() {
	...
	if len(os.Args) < 2 {
		text, keys, output, err := runTUI()
		if err != nil {
			logger.Fatal(err)
		}
		if err := export(text, output, keys); err != nil {
			logger.Fatal(err)
		}
		return
	}
	...
	if err := export(text, output, nil); err != nil {
		logger.Fatal(err)
	}
}
