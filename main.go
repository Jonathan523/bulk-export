...
func export(text string, keys []string, output string, firstRows []Row) error {
	chars := []rune(text)
...
func main() {
	...
	if len(os.Args) < 2 {
		...
		if err := export(res.text, res.keys, res.output, res.firstRows); err != nil {
			logger.Fatal(err)
		}
		return
	}
	...
	if err := export(text, keys, output, firstRows); err != nil {
		logger.Fatal(err)
	}
}
